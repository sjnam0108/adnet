...
<c:url value="/org/channel/playlist/create" var="createUrl" />
<c:url value="/org/channel/playlist/update" var="updateUrl" />
<c:url value="/org/channel/playlist/read" var="readUrl" />
<c:url value="/org/channel/playlist/destroy" var="destroyUrl" />

<c:url value="/org/channel/playlist/readAds" var="readAdUrl" />
<c:url value="/org/channel/playlist/editAdValue" var="editAdValueUrl" />
<c:url value="/org/channel/playlist/copyAs" var="copyAsUrl" />
...
<h4 class="pt-1 pb-3 mb-3">
	...
	<span class="mr-1 fa-light fa-list-ol"></span><span class="pl-1">재생목록</span>
</h4>
...
<ul class="nav nav-tabs tabs-alt mb-4 mt-3">
	...
	<li class="nav-item mr-auto">
		<a class="nav-link active" href="/org/channel/playlist/${Channel.id}">
...
$(document).ready(function() {
	...
	$("select[name='nav-item-chan-select']").on("change.bs.select", function(e){
		location.href = "/org/channel/playlist/" + $("select[name='nav-item-chan-select']").val();
	});
...
<%
	...
	String nameTemplate =
			"# if (code) { #" +
				"# if (code == 'C') { #" +
					"<span title='현재 재생목록' class='pr-1'><span class='fa-light fa-gear'></span></span>" +
				"# } else if (code == 'OC') { #" +
					"<span title='광고 편성중인 현재 재생목록' class='pr-1'><span class='fa-solid fa-gear text-blue'></span></span>" +
				"# } #" +
			"# } #" +
			"<span>#= name #</span>";
		...
		<kendo:dataSource-schema>
			<kendo:dataSource-schema-model id="id" >
				<kendo:dataSource-schema-model-fields>
					...
					<kendo:dataSource-schema-model-field name="totDurSecs" type="number" />
		...
		<form class="modal-content" id="form-1" rowid="-1" url="${createUrl}">
			...
			<div class="modal-body modal-bg-color">
				...
				<div class="form-group col">
					...
					<input name="endDate" type="text" class="form-control border-none">
				</div>
				<div class="form-group col">
					<label class="form-label">
						활성화 상태
...
function initForm1(subtitle) {
	...
	$("#form-1 input[name='endDate']").kendoDateTimePicker({
		format: "yyyy-MM-dd HH:mm", 
		parseFormats: [
			"yyyy-MM-dd HH:mm", "yyyy-MM-dd HHmm", "yyyy-MM-dd HH mm", "yyyy-MM-dd HH",
			"yyyyMMdd HH:mm", "yyyyMMdd HHmm", "yyyyMMdd HH mm", "yyyyMMdd HH",
...
function saveForm1() {
	...
	if ($("#form-1").valid()) {
    	var data = {
    		...
    		endDate: $("#form-1 input[name='endDate']").data("kendoDateTimePicker").value(),
...
function edit(id) {
	...
	$("#form-1 input[name='endDate']").data("kendoDateTimePicker").value(dataItem.endDate);
...
function initForm3() {
	...
	$("#form-3 input[name='endDate']").kendoDateTimePicker({
		format: "yyyy-MM-dd HH:mm", 
		parseFormats: [
			"yyyy-MM-dd HH:mm", "yyyy-MM-dd HHmm", "yyyy-MM-dd HH mm", "yyyy-MM-dd HH",
			"yyyyMMdd HH:mm", "yyyyMMdd HHmm", "yyyyMMdd HH mm", "yyyyMMdd HH",
...
function copyAs(id) {
	...
	$("#form-3 input[name='endDate']").data("kendoDateTimePicker").value(dataItem.endDate);
...
function saveForm3() {
	...
	if ($("#form-3").valid() && channel && channel != "-1") {
    	var data = {
    		...
    		endDate: $("#form-3 input[name='endDate']").data("kendoDateTimePicker").value(),
    	};
