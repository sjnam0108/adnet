...
<c:url value="/org/mediumopt/screen/read" var="readUrl" />

<c:url value="/org/mediumopt/screen/createWithNames" var="createWithNameUrl" />
<c:url value="/org/mediumopt/screen/createWithShortNames" var="createWithShortNameUrl" />
<c:url value="/org/mediumopt/screen/destroy" var="destroyUrl" />
...
<h4 class="pt-1 pb-3 mb-3">
	...
	<span class="text-muted font-weight-light">화면 옵션<span class="px-2">/</span></span>
...
<div class="card">
	<h6 class="card-header with-elements pl-2">
		<span class="lead">
			<span class="fa-stack fa-xs" title="화면 옵션" style="vertical-align: top; color: #a0a0a0;">
				<span class="fa-solid fa-circle fa-stack-2x"></span>
				<span class="fa-solid fa-sliders-simple fa-stack-1x fa-inverse fa-lg"></span>
			</span>
			<span>
				<span class="lead">${optID}</span>
				<span class='px-2'><small><span class='fa-solid fa-circle-small fa-2xs' style="color: #a0a0a0;"></span></small></span>
				${optName}
			</span>
		</span>
		<div class="card-header-elements ml-auto p-0 m-0">
			
			<button type="button" class="btn btn-outline-dark btn-round icon-btn btn-custom" id="qlb-medium-opt" title="일반 설정">
...
$(document).ready(function() {
	...
	// Quick Link Buttons
	$("#qlb-medium-opt").click(function(e) {
		e.preventDefault();
		
		location.href = "/org/mediumopt";
	});
	// / Quick Link Buttons
...
<c:if test="${fn:length(currOpts) > 0}" >

	<select name="nav-item-opt-select" class="selectpicker bg-white mb-1" data-style="btn-default" data-none-selected-text="" data-width="250px" data-size="10" >

<c:forEach var="item" items="${currOpts}">
...
$(document).ready(function() {
	...
	$("select[name='nav-item-opt-select']").selectpicker('render');

	$("select[name='nav-item-opt-select']").on("change.bs.select", function(e){
		location.href = "/org/mediumopt/screen/" + $("select[name='nav-item-opt-select']").val();
	});
	
	bootstrapSelectVal($("select[name='nav-item-opt-select']"), "${optID}");
			...
			showDelConfirmModal(function(result) {
				if (result) {
					$.ajax({
						...
						dataType: "json",
						url: "${destroyUrl}",
						data: JSON.stringify({ items: delRows, opt: "${optID}" }),
