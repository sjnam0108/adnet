...
<c:url value="/inv/syncpack/screen/read" var="readUrl" />

<c:url value="/inv/syncpack/screen/createWithNames" var="createWithNameUrl" />
<c:url value="/inv/syncpack/screen/createWithShortNames" var="createWithShortNameUrl" />
<c:url value="/inv/syncpack/screen/destroy" var="destroyUrl" />
<c:url value="/inv/syncpack/screen/sort" var="sortUrl" />
...
$(document).ready(function() {
	...
	// Quick Link Buttons
	$("#qlb-sync-pack").click(function(e) {
		e.preventDefault();
	...
	var packTitle = "<span>${SyncPack.name}</span>";
	
	$("#sync-pack-name").html(packTitle);
...
<kendo:grid name="grid" pageable="true" filterable="true" scrollable="true" reorderable="true" resizable="true">
	...
	<kendo:grid-toolbarTemplate>
    	<div class="clearfix">
    		<div class="float-left">
    			...
    			<button id="sort-btn" type="button" class="btn btn-default">
					<span class="fa-regular fa-arrow-up-a-z"></span>
					<span class="pl-1">이름순으로 정렬</span>
				</button>
	...
	<kendo:grid-columns>
		<kendo:grid-column title="번호" field="laneId" width="150" filterable="false" />
	...
	<kendo:dataSource serverPaging="false" serverSorting="false" serverFiltering="false" serverGrouping="false" error="kendoReadError">
		<kendo:dataSource-sort>
			<kendo:dataSource-sortItem field="laneId" dir="asc"/>
		</kendo:dataSource-sort>
				...
				<kendo:dataSource-transport-read-data>
					<script>
						function additionalData(e) {
							return { reqIntValue1:  ${SyncPack.id} };
						}
					</script>
				</kendo:dataSource-transport-read-data>
...
$(document).ready(function() {
	...
	$("#sort-btn").click(function(e) {
		...
		$.ajax({
			...
			dataType: "json",
			url: "${sortUrl}",
			data: JSON.stringify({ id: ${SyncPack.id}}),
		...
		$.ajax({
			...
			success: function (form, status, xhr) {
				...
				if (respText == "Ok") {
					...
					var msg = "다음의 요청 항목은 화면 미등록이거나 이미 다른 묶음에 포함된 것으로 확인되어 작업을 수행할 수 없습니다:<br>" + respText;
