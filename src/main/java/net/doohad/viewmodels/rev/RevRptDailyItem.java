...
public class RevRptDailyItem {
	...
	public RevRptDailyItem() {}
	
	public RevRptDailyItem(Date playDate) {
		this.playDate = playDate;
	}
	
	public RevRptDailyItem(Date playDate, int dataId) {
		this.playDate = playDate;
		this.dataId = dataId;
	}
	
	
	public float getActualValuePerScreen() {
		
		return (cntScreen == 0) ? 0 : (float)total / (float)cntScreen;
	}
	
	public void setTupleData(Tuple tuple) {
		if (tuple != null) {
			this.playDate = (Date) tuple.get(0);
